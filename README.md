...
- **Rules never touch the DOM.** Each game's logic has no DOM or Canvas calls and takes its random source as a parameter, so it is tested without a browser: **269 unit tests** across the game rules, shared modules and UI logic, plus **33 Playwright tests** in a real browser, all in CI.
