...
**8 个经典小游戏装进一台浏览器机柜。无框架，打包后 34 KB，全量测试。**
...
- **小。** 整个街机厅——8 个游戏、五子棋 AI、全部界面——打包后 **gzip 34 KB**，比一张普通截图还小。
- **零运行时依赖。** 构建期用 Vite + TypeScript；浏览器里只有原生 DOM 和 Canvas 2D。没有 React，没有游戏引擎。
- **每条规则都是纯函数。** 每个游戏的逻辑不碰 DOM 和 Canvas，可以脱离浏览器测试：**269 条单测**覆盖游戏规则、公共模块和界面逻辑，另有 **28 条 Playwright 端到端**在真实浏览器里跑，全部在 CI 里。
