...
- **规则层不碰 DOM。** 每个游戏的逻辑不引用 DOM 和 Canvas，随机源作为参数注入，可以脱离浏览器测试：**269 条单测**覆盖游戏规则、公共模块和界面逻辑，另有 **28 条 Playwright 端到端**在真实浏览器里跑，全部在 CI 里。
...
逻辑调用。这个项目做过一整轮视觉改版，动的是渲染代码、公共外壳和样式，八个 `logic.ts` 和它们的测试一行没改。
